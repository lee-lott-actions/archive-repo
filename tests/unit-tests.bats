...
# Load the Bash script containing the archive_repo function
load ../action.sh
...
@test "archive_repo succeeds with HTTP 200 and archived true" {
  ...
  run archive_repo "test-repo" "fake-token" "test-owner"

  [ "$status" -eq 0 ]
  [ "$(grep 'result' "$GITHUB_OUTPUT")" = "result=success" ]
}

@test "archive_repo fails with HTTP 404" {
  ...
  [ "$(grep 'error-message' "$GITHUB_OUTPUT")" = "error-message=Failed to archive repository. HTTP Status: 404" ]
}

@test "archive_repo fails when archived is false" {
  ...
  [ "$status" -eq 0 ]
  [ "$(grep 'result' "$GITHUB_OUTPUT")" = "result=failure" ]
  [ "$(grep 'error-message' "$GITHUB_OUTPUT")" = "error-message=Failed to archive repository. HTTP Status: 200" ]
}

@test "archive_repo fails with empty repo_name" {
  run archive_repo "" "fake-token" "test-owner"
...
@test "archive_repo fails with empty token" {
  run archive_repo "test-repo" "" "test-owner"
...
@test "archive_repo fails with empty owner" {
  run archive_repo "test-repo" "fake-token" ""
